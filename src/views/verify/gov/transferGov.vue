<template>
  ...
  >
      ...
      <n-form label-placement="left" inline @keypress.enter="tableElRef?.handleSearch">
        <n-grid cols="4" :x-gap="20" :collapsed="gridCollapsed" responsive="screen">
          ...
          <n-form-item-gi label="审核状态：">
            <n-select v-model:value="formData.govApprovalStatus" :options="verifyEnum" clearable />
          </n-form-item-gi>
    ...
    <TransferInfoModal @register="register" @success="tableElRef?.handleSearch" />
  </q-list>
</template>
...
import TransferInfoModal from './components/TransferInfoModal.vue';
import { transferColumns, verifyEnum } from './index.data';
