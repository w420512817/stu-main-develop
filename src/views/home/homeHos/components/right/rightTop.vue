<template>
  <div class="card">
    <div class="flex justify-between items-start mb-15px h-32px">
      <div class="tit flex-shrink-0">消息通知</div>
      <div class="flex flex-1 justify-end">
        <n-button v-auth="4059" type="primary" style="--n-height: 32px" @click="go({ name: 'sendModify' })">
          发布通知
        </n-button>
      </div>
    </div>
    <n-scrollbar class="h-164px">
      <div class="msg-list flex-col gap-y-3">
        <div
          v-for="item in msgList"
          :key="item.tit"
          class="msg-box flex items-center justify-between"
          title="查看消息"
          @click="goMsg(item)"
        >
...
import { noticeList } from '@/api/dashbord/hosptial';
