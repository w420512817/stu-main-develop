<template>
  <n-card class="h-full shadow-sm" :title="`${query.regionName} - 医院`">
        ...
        <n-form inline :model="formValue" label-placement="left" @keypress.enter="onSearch">
          <n-grid cols="2 md:3 xl:4 2xl:5 3xl:6" :x-gap="20" :collapsed="gridCollapsed" responsive="screen">
            ...
            <n-form-item-gi label="医院类型:">
              <n-select
                v-model:value="formValue.hospType"
                :options="hosptialTypesOptions"
                placeholder="请选择医院类型"
                @update:value="onSearch"
              />
            </n-form-item-gi>
...
import { hosptialAttrOptions, hosptialTypesOptions, hosptialLevelOptions } from '../../institution/common.data';
...
// api
import { getHosptialList } from '@/api/institution/hosptial';
import { exportHospitalList } from '@/api/institution';

// init hooks, static data

const { fetch: expApi, loading: expLoading } = useApi(exportHospitalList);
