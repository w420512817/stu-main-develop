...
import { getAbnormalResult } from '@/api/bigScreen/phyExam';
      ...
      return `<div class="tooltip-large">
                ...
                <div class="content">
                  <div class="line mt-0.6vh">
                    <span>肥胖率</span>
                    <span class="text-[#F3C549]">${numToPercent(p.data?.obesityRate)}%</span>
                  </div>
                  <div class="line mt-0.6vh">
                    <span>营养不良率</span>
                    <span class="text-[#27DDFF]">${numToPercent(p.data?.malnutritionRate)}%</span>
                  </div>
                  <div class="line mt-0.6vh">
                    <span>脊柱侧弯不良率</span>
                    <span class="text-[#27DDFF]">${numToPercent(p.data?.abnormalSpineRate)}%</span>
                  </div>
                </div>
              </div>`;
    }
...
async function getAreaMapData() {
  // 重新获取地图区域数据
  const res = await getAbnormalResult({ planId: params.planId, id: params.id, level: params.level });
  cacheFeatures.forEach(item => {
    const temp = res.find(i => i.areaId === item.id);
    item.obesityRate = temp?.obesityRate || 0;
    item.malnutritionRate = temp?.malnutritionRate || 0;
    item.abnormalSpineRate = temp?.abnormalSpineRate || 0;
  });
...
.tooltip-large {
  ...
  background-image: url('@/assets/image/bigScreen/tooltip-large.png');
  background-size: 100%;
  background-repeat: no-repeat;
