...
import { govDentalPeripheryCondition } from '@/api/bigScreen/cavity';
...
async function getData() {
  try {
    loadingFn(1);
    const res = await govDentalPeripheryCondition({ planId: params.planId, id: params.id, level: params.level });
    // console.log('right-mid', res);
    opts.xAxis.data = res.map(i => i.areaName);
    opts.series[0].data = res.map(i => numToPercent(i.manPeriodontitisRate));
    opts.series[1].data = res.map(i => numToPercent(i.womanPeriodontitisRate));
    opts.series[2].data = res.map(i => numToPercent(i.periodontitisRate));
