<template>
  <SubTitle :sub-title="`${areaTitle}体测预警数据`" />
...
import { programWarning } from '@/api/bigScreen/phyTest';
...
async function getData() {
  try {
    loadingFn(1);
    const res = await programWarning({ planId: params.planId, id: params.id, level: params.level });
    // console.log('right-bottom', res);
    opts.yAxis[0].data = opts.yAxis[1].data = opts.yAxis[2].data = res.map(item => item.programName);
    opts.series[0].data = res.map(item => item.boyWarningCount);
    opts.series[1].data = res.map(item => item.girlWarningCount);
    setOptions(opts);
  } finally {
    loadingFn(-1);
  }
