    ...
    formatter: function (p) {
      return `<div class="tooltip-small">
                ...
                <div class="content">
                  <div class="line">
                    <span>参与人数</span>
                    <span class="text-[#F3C549]">${p.data?.effectiveStdCount || 0} <i class="unit">人</i></span>
                  </div>
                </div>
              </div>`;
    }
...
async function goBack() {
  ...
  // 重新获取地图区域数据
  const { geoVOList } = await getMapData({ planId: params.planId, id: params.id, level: params.level });
  // console.log('map-data', geoVOList);
  cacheFeatures.forEach(item => {
    const temp = geoVOList.find(i => i.areaId === item.id);
    item.effectiveStdCount = temp?.effectiveStdCount || 0;
  });
