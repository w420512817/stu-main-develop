<template>
  ...
  <div class="expand-all">
    <div class="relative wh-full">
      ...
      <div class="w-14vh h-14vh absolute circle1">
...
import { screenOverview } from '@/api/bigScreen/phyTest';
...
function setPie() {
  ...
  setOptions({
    title: {
      text: `${(resObj.value.totalRate * 100).toFixed(2)}%`,
      subtext: '总筛查率',
      textStyle: {
        fontSize: 22,
        color: '#CDEFFB',
...
async function getPlanList() {
  try {
    loadingFn(1);
    const { records } = await planList({ appType: screenTypeMap['体测'] });
...
async function setBar() {
  const res = await screenOverview({ planId: currentBatch.value, id: params.id, level: params.level });
