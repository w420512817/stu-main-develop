        ...
        <table class="table-temp" w-full cellspacing="0">
          <tbody>
            ...
            <tr>
              ...
              <td>
                <span>
                  胸&emsp;段： {{ sourceDataEnum.anteriorFlexionThoracic[item.anteriorFlexionThoracic] || '/' }}
                </span>
              </td>
            </tr>
            <tr>
              <td>
                <span>
                  腰胸段： {{ sourceDataEnum.anteriorFlexionWaistChest[item.anteriorFlexionWaistChest] || '/' }}
                </span>
              </td>
            </tr>
            <tr>
              <td>
                <span>
                  腰&emsp;段： {{ sourceDataEnum.anteriorLumbarFlexion[item.anteriorLumbarFlexion] || '/' }}
                </span>
              </td>
            </tr>
                ...
                <div flex>
                  ...
                  <div lh-8>
                    ...
                    <div>
                      ...
                      <span> 胸段ATR：{{ sourceDataEnum.thoracicSegmentAtr[item.thoracicSegmentAtr] || '/' }} </span>
                      <span>
                        &nbsp;&nbsp;腰胸段ATR：{{
                          sourceDataEnum.thoracolumbarSegmentAtr[item.thoracolumbarSegmentAtr] || '/'
                        }}
                      </span>
                      <span
                        >&nbsp;&nbsp;腰段ATR：{{ sourceDataEnum.lumbarSegmentAtr[item.lumbarSegmentAtr] || '/' }}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <tr>
              <td colspan="2">
                ...
                <span>{{ sourceDataEnum.proneTest[item.proneTest] || '/' }}</span>
              </td>
            </tr>
              ...
              <td colspan="2">
                <div>
                  ...
                  <div flex lh-8>
                    ...
                    <div>{{ item.suggestion || '保持健康生活方式' }}</div>
                  </div>
...
import { sourceDataEnum } from './index.data';
...
import { printScreeningReport } from '@/api/information/student';
...
async function onDataReceive(data) {
  ...
  changeLoading();
  try {
    const res = await printScreeningReport(params);
    res.forEach(item => {
      ...
      item.bendGeneralInspection = item.bendGeneralInspection
        ?.split(',')
