          ...
          <n-card border border-gray-100 mr-4>
            ...
            <div v-if="state.successCount" flex justify-between>
              <div>
                ...
                <p flex>
                  <span mr-3>学段：{{ state.schoolTypeName }}</span>
                  <span>学生类型： {{ state.taskTypeName }}</span>
                </p>
                <p>
                  新增
              ...
              <n-progress :stroke-width="8" style="width: 70px" type="circle" status="success" :percentage="100">
                <span text-success text-13px>{{ state.completion }}</span>
              </n-progress>
            </div>
            <!--导入全部失败-->
...
import { exportErrorData } from '@/api/information/student';
...
const state = computed(() => {
  console.log(studentStore.state, 'studentStore.state');
...
function handleImportBtn() {
  studentStore.showImportBtn();
  studentStore.hideResultModal();
}
async function handleDownload() {
  changeLoading();
  try {
    const res = await exportErrorData({ taskId: studentStore.state.taskId || null });
    res && downloadFile(res.data, '学生导入失败信息表', 'xlsx');
  } catch (e) {
